import { Body, Controller, Delete, NotFoundException, Post, UnauthorizedException, UseGuards } from '@nestjs/common';
import { DeleteResponseInterface, JwtPyloadInterface } from '@domotica/shared/interfaces';
import { CONTROLLER_AUTH_PATH, CONTROLLER_BASE_PATH, RESOURCE, RESPONSE_STATUS } from '@domotica/shared/enums';
...
export class AuthController {
  ...
  async signOut(@AuthUser() auth: JwtPyloadInterface): Promise<DeleteResponseInterface> {
  ...
  async refresh(@Body() dto: { refreshKey: string }): Promise<SignInResponseDto> {
